// todo: if anything about data is specified, don't use defaults at all
...
var gimme = {
	...
	get: function (shoppingList, useJsonp) {
		...
		var getHandler = function(resolve, reject) {
			...
			if (shoppingList.length > 0) {
				for(var i=0; i<shoppingList.length; i++){
					...
					var item = inventory[ shoppingList[i].gimme ]( shoppingList[i].data );

					// don't use defaults if any data has been defined
					if(shoppingList[i].data)
						delete item.data;

					// merge into single item
					var item = merge( item, shoppingList[i] );
		...
		return new Promise((resolve, reject) => {
			...
			var retryAtMost = (maxRetries, promise) => {
				...
				promise.then((data) => {
					...
					if (maxRetries > 0) {
						var tryRefresh = () => {
							fetch('https://secure.meetup.com/oauth2/access?' + 'client_id=i8ofuea4hi35ro573qeck6j15d&client_secret=bq4u0j021mbh4pne3s18oup3vf&grant_type=refresh_token&refresh_token=' + _this.refreshToken, {
				        method: 'POST'
				      }).then((response) => {
				        return response.json();
				      	...
				      	retryAtMost(maxRetries - 1, new Promise(getHandler));
				      });
				     }
			...
			retryAtMost(5, new Promise(getHandler));
